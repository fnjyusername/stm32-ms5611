...
u16 FactoryCalculation[7];//用来存放出厂校准数据C1~C6.为什么数组给7位?见Datasheet第12页,其中第0位被 "16 bit reserved for manufacturer ",实际有效参数是从第1位到第6位
...
void MS5611_ReadPROM(void)
{
	...
	for(i=0;i<7;i++)//共需读七个数据,但第0个是无效的
	{
		MS5611_PIN_CSB_BIT=0;//拉低片选
		
		MS5611_ReadWriteByte(CMD_MS5611_PROM_READ+i*2);//这条语句的作用是写地址到芯片中,这样芯片就能"准备"数据了,接下来就可以读数据了
